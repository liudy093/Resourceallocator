...
func clientTaskCreatePod(request *resource_allocator.CreateTaskPodRequest, clientService *kubernetes.Clientset,
	...
	if os.Getenv("RESOURCE_ALGORITHM") == "adaptive" {
				...
				v1.Container{
					...
					Resources: v1.ResourceRequirements{
						Requests: v1.ResourceList{
							...
							v1.ResourceMemory: resource.MustParse(strconv.Itoa(int(memWeight*float64(request.Mem))) + "m"),
						},
		...
		log.Println("This is a normal task pod with adaptive.")
	}
				...
				//GPU任务需扩大shm
				{
					...
					VolumeSource: v1.VolumeSource{
						EmptyDir: &v1.EmptyDirVolumeSource{
							Medium:    "Memory",
							SizeLimit: resource.NewQuantity(32*1024*1024*1024, resource.BinarySI), // 32Gi
						},
					},
				},
			},
	...
	serviceRequired := exists && serviceValue == "1"
	if serviceRequired {
		// 创建Service
		service := &v1.Service{
